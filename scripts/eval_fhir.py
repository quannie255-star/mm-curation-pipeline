...
def main() -> int:
    ...
    plan = ContaminationPlan(inject_rate=args.inject_rate, seed=args.seed, kinds=dict(FHIR_KINDS))
    ...
    print(f"\n医疗 FHIR P/R: {config.name}（{len(samples)} 条全集, {len(config.operators)} 算子）")
