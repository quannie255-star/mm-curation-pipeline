"""医疗 FHIR 模态评测（V4 α）：算子级 P/R + 漏斗串联门禁。
...
与单元测试的分工（data_ci 同哲学）：单测验证「逻辑对」，本脚本门禁验证
「合成语料上的质量数字达标」——算子劣化（哪怕测试仍绿）门禁会红。

用法：
    python -X utf8 scripts/eval_fhir.py                        # 全量 500 条 + 门禁
    python -X utf8 scripts/eval_fhir.py --scale 0.1            # 冒烟档（50 条）
    python -X utf8 scripts/eval_fhir.py --no-gate              # 只出报告不变红
"""
...
def funnel_gate_metrics(dropped, n_dirty: int, n_clean: int) -> dict:
    """漏斗串联口径的门禁指标：总体故障召回 + 干净误杀率。

    dropped 为 FunnelResult.dropped（(算子名, 样本) 元组）。
    """
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/funnel_fhir.yaml")
    parser.add_argument("--out", default="data/reports/operator_pr_fhir.json")
    ...
    last_plan = ContaminationPlan(
        inject_rate=args.inject_rate, seed=last["seed"], kinds=dict(FHIR_KINDS)
    )
    last_mixed, manifest = last_plan.run(last_corpus, Path("data/tmp_fhir_images"))
    ...
    print(
        f"\n医疗 FHIR P/R: {config.name}（{len(last_mixed)} 条全集, {len(config.operators)} 算子）"
    )
...
def _gate_markdown(gate: dict, args, runs: list[dict]) -> str:
    return "\n".join(
        [
            ...
            "",
            "> 语料为程序生成的合成 FHIR R4 数据，不含任何真实患者数据；",
            *(
                [
