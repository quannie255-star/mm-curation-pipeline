...
**W2 收尾状态（2026-09-20 实点）**：W2-1 ~ W2-5 **全部完成**；W2-6 **已跑出真实数字**
（60 篇真实页面 → `data/reports/extract_ablation.{json,md}`：站点档 49/60、兜底器 60/60、
`trafilatura` 未装记 `unavailable` 并**从分母剔除**；救回率 **100%（11/11）**、丢失率 **0%**，
但字数比 **1.92×**、多抽段占比 **88.9%** → **兜底器定位 = 保证从不失败，不是更好的抽取器**）。
**W2-6 的人工抽检 30 篇只产出对照表**（`data/reports/extract_review_sample.jsonl`，
`human_verdict` 留空待填）——报告明写「不含人工质量判定」。
**新基线：主仓 387**（328 + `test_extract.py` 33 + `test_connectors.py` 26）**+ 包 67**，
0 失败/0 错误/0 跳过；ruff 双仓全绿。
**遗留待办**：站点档 `<a>` 剥离正则的两个缺陷（段内行内链接文本被删 / 跨段拼段）
会改变既有语料，须配独立 A/B 后再修（见 `docs/ENGINEERING_NOTES.md` #66）。
...
2. **零破坏**：既有 4 个 config 的数字逐项相等（归一化 opt-in 未启用时）；
   测试基线不倒退（**2026-09-20 实点：主仓 328 + 包 67，其中包侧 5 条为 ray 测试，
   ray 2.58.0 已装；未装 ray 的环境/CI 包侧为 62**）。
...
## 裁决结果（2026-09-20，三处**全部按建议通过**）

用户原话：「按照你建议的来，至于怎么做出差异化，我觉得先把全链路搭出来再说，
搭出来再看看怎么引入新的前沿技术」——即：三处按建议执行；**差异化定位明确挂起**，
本轮不为它额外投入。W1 已据此收官（`70a4e7e`）。

| # | 决策 | 结论 | 落地情况 |
|---|---|---|---|
| **1** | 可视化定位承认「DJ 已有」后，是否仍投入 4 页签 | ✅ **是**，叙事改写为「我做的三件事 DJ 没做」 | W1 先落 2 个（⑦清洗过程 / ⑧阈值沙盘）；剩余 ⑨人审队列 / ⑩迁移代价 随 W3/W4 |
| **2** | 归一化层默认开还是 opt-in | ✅ **opt-in**（既有 config 一字不动） | W1 已落；`run_funnel(pre_stages=…)` 默认空，V4「逐项相等」红线未破 |
| **3** | UI 是否搬进 pip 包（W5） | ✅ **是** | W5 执行 |

---

# V6 补表 v2：JD 驱动的能力对齐与增补（2026-09-20）

> **触发**：用户指示「对齐相关岗位的 JD 要求，继续完善项目」。
> **依据**：`docs/JD_RESEARCH.md` v2（同日重抓 10 个在招岗位 → 18 条要求聚类）。
> **本表只登记「V6 未覆盖」的新增项（B 档）**；A 档（采集解析/血缘/配比/checkpoint/UI）
> 已由 W2–W5 覆盖，不重复登记。
> ⚠️ **按设计门：本表所有条目等用户确认后才动码。** 本轮实际动码的只有已批准的 W2。

## 一、选题依据（为什么是这四项）

JD 聚类结果：18 条要求里 **✅ 有实证 8 条 / 🟡 部分具备 7 条 / 🔴 缺 3 条**。
三条 🔴 是 **数据合成与增强**（5/10 岗位）、**视频音频模态**（4/10）、**SQL / 数据建模**（3/10）
——**没有一条是清洗能力本身，全是能力外延**。

> 这个结论本身就值得写进叙事：**清洗这条线已经够深（29 算子 / 四模态 / P:R 可量化，
> 是 ✅ 最密集的一栏），下一步该补的是出口，不是继续加算子。**

## 二、任务增补表

| # | 任务 | JD 依据 | 与现有架构的关系 | 验收（必须出数，禁止"层补好了但没数字"） |
|---|---|---|---|---|
| **J1** | **合成 / 增强通道（1→N 扩样）** | 阿里云「数据合成 Data Synthesis」；基础设施岗「数据合成与增强管线工程化」；阿里 Astar「清洗、去重、**增强**」；阶跃「合成数据、自蒸馏数据」 | ⚠️ **需要协议变更**：W1 的 `Transformer` 只能 1→1，合成/增强是 1→N | 合成通道 ≥10 单测；**A/B/C 实验**：种子集 / +合成 / +增强 三口径下游 ΔR@1；**阴性出口**：无提升则如实记录并改写定位为「稀有域补样成本下降」 |
| **J2** | **视频 / 音频模态 + 抽帧** | 4/10 岗位（B站/阿里云/基础设施岗）；「FFmpeg/OpenCV/Librosa、视频抽帧与时序采样」 | 复用现有图像算子链（抽帧后即图像问题）→ 扩模态的**低成本路径** | 抽帧确定性（同视频同参数逐帧 md5 一致）；≥5 单测；对抽帧后帧跑现有图像漏斗出一条漏斗水位 |
| **J3** | **数据目录 + SQL 查询层** | 阿里云「数据资产化/数仓/建模」；AI 数据工程师「MySQL/索引优化/版本快照」；字节「SQL」 | 在 jsonl/parquet 上开 DuckDB 视图；**判决书台账天然适合 SQL 聚合** | ≥6 单测；用 SQL 复现一个现有报告数字（**逐位相等**才算过），证明它可替代手写聚合 |
| **J4** | **语料分布体检报告**（多样性/冗余/缺失/长尾） | B站「分析海量数据分布、多样性、冗余、缺失及长尾」；阿里云「数据挖掘分析」 | 把散落片段（去重冗余率 / 长度分布 / 空白双峰 / PSI）收成一份体系化报告 | ≥5 单测；在 2066 篇真实语料 + 300k 维基语料上各出一份，指标全部可复现 |
| **J5** | **公开评测生态互通（导出/导入）** | 字节「把公开评测集集成至内部平台」；3/10 岗位点 OpenCompass/LMMS-Eval | **不做全套集成**，只做互操作层 | ≥4 单测；导出→重新导入→**评测结果逐位不变**（往返一致性） |

## 三、J1 设计要点（唯一需要协议变更的一项，三表如下）

### 3.1 数据结构表

| 字段 / 类型 | 序列化 | 消费方 |
|---|---|---|
| `SynthResult` · `produced: list[Sample]` | 内存对象（不落盘） | `run_pre_stages` 的扩样收集器 |
| `SynthResult.provenance: str`（必需） | 落进每个产出样本的 `meta["synth:<stage>"]` | 判决书 `input_fingerprint` 溯源、配比层去重记账 |
| `SynthResult.reason` | 同上 | 报告聚合（"为什么这条被扩出来"） |
| `Sample.meta["synth:origin_id"]` | str | 血缘：产出样本必须能指回种子 id |

### 3.2 接口约定表

| 接口 | 约定 |
|---|---|
| `Synthesizer`（ABC，与 `Operator`/`Transformer` **并列**，非继承） | `name: str`（唯一，注册期 fail-fast）、`modalities: frozenset[str]`、`synthesize(sample) -> SynthResult \| None` |
| `run_pre_stages(samples, transformers, synthesizers)` | 顺序：**扩样先于改写**（改写不该对未过滤的合成样本做重复功） |
| CLI `scripts/synth_ablation.py` | `--config`（`./configs/*.yaml`）`--limit N` `--out-dir`；退出码 0 正常 / 1 断言失败 / 2 参数错误 |
| 默认行为 | `synthesizers=()` 时 **逐位等于旧行为**（与 `pre_stages` 同一条红线） |

### 3.3 流转表

```
种子 Sample(clean)
  → [Synthesizer] 0 个产出 = 不扩样（合法，记 reason="no_transform"）
  → [Synthesizer] N 个产出 = 产出样本 origin_id 指向种子，进入后续全部打分
  → 现有 Operator 漏斗（**合成样本与真样本走同一条漏斗**——这是关键的诚实设计：
     合成质量由下游算子裁决，不由合成器自评）
  → 判决书（合成样本与真样本同 schema，靠 provenance 区分）
```

> **一条红线**：合成样本**不允许绕过漏斗**。否则"我合成了一批数据"就变成无法证伪的宣称——
> 只有让它和真数据一起被同一套判据裁决，才谈得上质量。

## 四、明确不追（C 档，写下来比装看不见强）

| 不追 | 理由 |
|---|---|
| EB / PB 级实跑 | 个人无算力。定位是「单机跑通 + 同一算子图一行切 Ray + 如实标注回本条件」 |
| Agent 行为数据 / RLHF 全链路 | 偏离「数据质量」主线；只保留 LLM-judge 与偏好判官作为相关证据 |
| 论文级产出 | 不做 |
| PE / Agent 应用开发 | 不是本项目定位 |

## 五、执行顺序建议

```
W2（抽取层，已批准，进行中）
  → J1（合成/增强通道：最大的新架构缺口，JD 出现频次最高）
  → J3 / J4（小、便宜、演示价值高，可随时插入）
  → W3（人审层）→ W4（配比 + 血缘）→ W5（进包 + 生态）
  → J2（视频音频：最重，需新语料与 ffmpeg）→ J5（生态互通）
```

---

# P2 补表：真实分布适用性修复 + 真实数据交互前端（2026-09-20）

> **触发**：用户指示「先解决处理清洗真实工业数据效果不好这一问题，其他的可以先放一放，
> 然后给我一个漂亮的可交互前端」。即暂停 J1–J5 / W3–W5 / 面试收尾包。
> **依据**：`docs/REAL_DATA_REPORT.md`（三数据集首跑）+ `docs/GAP_AUDIT.md` §六 P2 档。
> **范围**：工业传感器模态的**判据 ↔ 真实形态对齐** + 一个可交互的真实数据前端。
> ⚠️ **本表按设计门等用户确认后才动码。** 判据变更会改变算子行为与合成门禁数字，
> 因此「既有数字如何处理」是本表的一等公民，不是脚注。

## 一、问题实点（代码级，不是转述文档）

读完 `industrial_quality.py`（277 行）与 `scripts/ingest_real_sensor.py` 后，
五种失效可以归成三类，每一类都能指到具体代码行：

| 算子 | 现判据（代码位置） | 真实数据上发生了什么 | 失效类 |
|---|---|---|---|
| `sensor_stuck` | L73 `max-min <= 1e-9` 且 `operating_mode != "idle"` | SKAB 全静默（真实传感器有噪声，窗内**不严格恒定**）；C-MAPSS 7817 次误杀里 **7220（92.4%）**落在 6 个**全程恒定**的无信息通道 | **F2 同形态多含义** |
| `sensor_range` | L95-97 表外通道 `return 1.0` | 真实 `device_type`/`channel` 不在内嵌表 → 恒 1.0 → 报告显示「通过」，实际**从未评过** | **F3 伪装成通过** |
| `sensor_drift` | L142-148 前 5 窗建基线 + pooled σ×AR(1)；L166-167 `abs(z) > 4` | 真实误杀 16.3%~28.3%，且**任何阈值都救不回**（放宽到 8σ 才压到 27%，召回掉到 34.5%） | **F2 模型假设错** |
| `unit_consistency` | L214 组内单位众数 | 真实单位是从数据集文档**统一填**的，无混源 → 恒过 | **F1 空转** |
| `fault_vs_maintenance` | L263 `all(v == SENTINEL)` | 真实链路故障表现为「**没有数据**」，不是一条 -999 | **F1 空转** |

**根因一句话**：合成污染器注入的是**判据的形状**（完美平坦 / 哨兵 / 表内量程），
真实数据里这些形状要么不存在，要么**同一个形状另有含义**（恒定 ≠ 卡死）。
所以修法不是「调阈值」——REAL_DATA_REPORT §一 的五档阈值扫描已经证明调不动。

**真实轨标签的现实（必须写进验收口径）**：`ingest_real_sensor.py` 产出的标签是
`sensor_anomaly` / `fault_window_*` / `degraded`，**与 `OPERATOR_TARGETS` 的
`sensor_flatline` 等合成类型名不对应**。因此真实轨只有**粗粒度召回**（能不能命中
数据集标注的异常段）与**误杀率上界**（无标签 ≠ 干净，MetroPT-3 163 小时全通道冻结
被官方标签漏掉就是实证）。

## 二、六项修复设计（R1–R6）

每项给出：**改什么 → 为什么这么改 → 兼容性 → 验收数**。

### R1 `sensor_stuck`：从「单窗完美平坦」到「持续尺度塌陷」

| 项 | 内容 |
|---|---|
| 判据 | ①**无信息通道豁免**：该通道在**全数据集**上恒定 → 不判卡死（`return None`）②**持续塌陷**：窗内尺度 `σ_win` 显著低于该通道全局尺度，**且连续 ≥k 窗**如此 → 判卡死（冻结是持续现象，单窗低方差是噪声）③保留 `idle` 工况豁免 |
| 实现 | `Operator` → `BatchOperator`（`shardable=False`，与 `sensor_drift` 同款）。**注册名/参数/`Sample` 协议一字不动**，config 不用改 |
| 为什么 | 直接针对 C-MAPSS 92.4% 误杀（豁免）与 SKAB 全静默（塌陷判据比 `≤1e-9` 灵敏）；「连续 k 窗」是区分「冻结」与「合法低方差工况」的唯一可靠信号 |
| 验收 | C-MAPSS `stuck` 误杀 26.27% → 目标 **≤5%**；SKAB 默认窗下 `stuck` **从 0 判决变为有判决**（空转消失）；合成轨 stuck 召回 **保持 100%** |

### R2 `sensor_range`：表外通道记 `None`，并让量程表可外接

| 项 | 内容 |
|---|---|
| 判据 | 表外通道 `return None`（无法计分）而非 `1.0`。漏斗行为不变（`None` 语义 = 保留并记录），**变的只是报告不再把「没评」说成「通过」** |
| 新增 | 可选 `params.ranges_path`（YAML/JSON 外部量程表），缺省仍用内嵌 `RANGE_TABLE`。**纯增量参数** |
| 诚实约束 | 真实量程**必须可追溯**（数据集文档 / 传感器手册），查不到的通道**留空** → `None`。**禁止为了好看硬填** |
| 验收 | 真实轨报告出现 `None` 计数（「未评通道数」），SKAB 8 通道中可接的通道有判决；合成轨数字**不变**（表内命中） |

### R3 `fault_vs_maintenance`：静默 = 槽位缺席（并保留哨兵兼容）

| 项 | 内容 |
|---|---|
| 架构落点 | **槽位缺席在漏斗外前置阶段解决**（符合既有原则「窗口化 + 标签聚合是前置阶段」）：`ingest_real_sensor.py` 按 `sampling_hz` × 窗长推算**应有槽位网格**，缺失槽生成 `record_type: "gap_window"` 样本（带 `missing_slots` 字段） |
| 判据 | 算子认**两种**静默形态：①窗内全哨兵（合成，**保留兼容**）②`gap_window` 且不在检修计划内 → 链路故障 |
| 为什么 | 真实链路故障是「没有数据」。若直接**替换**哨兵判据，合成门禁的 `sensor_unplanned_silence` 立刻失效——所以是**并集而非替换**，这是本表的一条红线 |
| 验收 | MetroPT-3 上 `fault_vs_maintenance` **从全静默变为有判决**；合成轨 `unplanned_silence` 召回**保持 100%** |

### R4 `sensor_drift`：换尺度估计（MAD），并出整条曲线

| 项 | 内容 |
|---|---|
| 判据 | 尺度从「前 5 窗 pooled σ × AR(1) 因子」换成**全组窗均值的 MAD**（稳健尺度，不依赖「最早 5 窗是稳态」这个在真实数据里不成立的假设）；中心仍取中位数 |
| 标定 | 阈值 `k` 按**合成干净集零误杀**标定（保证合成门禁不回退），**然后如实报真实轨误杀上界** |
| 必出的数 | 不只报单点：出**整条 召回-误杀 曲线**（三数据集 × k 网格）。读者要能看到「换模型假设后曲线是整体上移，还是只是平移」——这才是「模型假设对不对」与「阈值定得松紧」的分界 |
| 验收 | 合成轨召回 100% 不回退；真实轨误杀**在召回不塌（≥45%）前提下**较现状下降（目标：MetroPT-3 28.3% → ≤20%，C-MAPSS 16.3% → ≤12%） |

### R5 合成污染器补「真实形态」（`packages/curation-eval/.../sensor_contamination.py`）

补三类形态 + 一个档位。**注意第 ③ 类是负样本**，它进的是**干净集**：

| # | 形态 | 进哪一集 | 用途 |
|---|---|---|---|
| ① | `noisy_flatline`（噪声平坦 / 抖动冻结） | 脏集 | 让 `sensor_stuck` 的塌陷判据**在合成轨也有靶子**，否则 R1 的灵敏度无法回归 |
| ② | `sampling_stall`（采样停摆 / 心跳缺失 → gap 记录） | 脏集 | 给 R3 的 `gap_window` 通路配合成靶子 |
| ③ | `uninformative_channel`（全程恒定但**合法**） | **干净集** | **误杀率只有把「合法恒定」放进干净集才测得出**——这是 C-MAPSS 92.4% 误杀在合成轨**完全测不到**的根因 |
| ④ | 低脏率档（~2%） | 档位参数 | 合成注入率 30% vs 真实 1.97%，**差 15 倍**；高脏率下阈值严重乐观 |

### R6 真实轨抽检工具 + 阈值扫描（固化成脚本能力）

| 能力 | 接口 | 解决什么 |
|---|---|---|
| 误杀清单导出 | `eval_real_sensor.py --export-kills <csv>` | 「误杀率只是上界」这句结论，现在靠临时脚本才查得动 → 固化成**通道 + 时间 + 读数极值 + 分数**表，人工可一键裁决（REAL_DATA_REPORT §五 第 5 条） |
| 阈值扫描 | `eval_real_sensor.py --sweep <op>` | 把「调参无解」从一次性探针变成**可重复的曲线**，R4 的验收数由它产出 |
| 未评通道统计 | 报告新增 `n_unscored_channels` | R2 的验收数由它产出，且让「伪装成通过」永久可见 |
...
| 算子 | 合成轨数字是否变化 | 依据 |
|---|---|---|
| `sensor_stuck` | **不变** | 合成每通道有噪声、无恒定通道 → 豁免不触发；塌陷判据需与全局尺度比，合成卡死是**严格**平坦，必然触发 |
| `sensor_range` | **不变** | 合成走表内；`None` 只影响表外 |
| `fault_vs_maintenance` | **不变** | 哨兵通路保留兼容（并集） |
| `unit_consistency` | **不变** | 未改动 |
| `sensor_drift` | ⚠️ **会变** | 尺度估计换了 → 阈值 `k` 必须重标定 |

**处理纪律**：
1. `sensor_drift` 的 `k` 按**合成干净集零误杀**标定后重跑合成门禁，**实点回写**新数字；
2. 若新数字较现基线（召回 100% / 误杀 1.23% 最差 seed）**回退**，视为 R4 未通过，
   回退方案 = 保留 `pooled σ` 作为默认、把 MAD 变成 `params.scale="mad"` 可选档
   （**保底线：既有 config 一字不动**）；
3. **⚠️ 协作阻断点**：`docs/PROOF_CHAIN.md`（工业召回 100% / 误杀 1.23%）与
   `docs/claims.json` **当前是 glm 的在途改动**。数字若变，**我不提交这两个文件**，
   而是在 `DEV_PLAN.md` 里明确写出「待 glm 同步的数字」清单，避免吞掉对方工作区。

## 四、真实性验收标准（新增，独立于合成门禁）

合成门禁是**回归红线**（防倒退），真实轨是**有效性证据**（证明能用）。两者不可互相顶替。
本表给真实轨设**三条硬指标**，达不到就如实报不到：

| # | 硬指标 | 判据 | 为什么是这条 |
|---|---|---|---|
| **V1** | **空转消失** | 五个算子 × 三个真实数据集，`n_dropped == 0` 的格子数**降为 0** | 现状 SKAB 默认窗下 **5 算子全 0**——「不误杀」是因为**根本没在判**，这是最危险的假绿 |
| **V2** | **误杀上界下降** | 同一数据集同一窗宽，误杀率较 REAL_DATA_REPORT 基线下降，**且召回不塌** | 上界要标清楚：无标签 ≠ 干净 |
| **V3** | **每处判据变更可归因** | 每条判据改动配一条**记录级证据**（抽检导出里能被人工核到的具体通道/时段） | 防止「改到数字好看但没人知道为什么」 |

**明确不宣称**：真实轨数字**不进 `claims.json`**（REAL_DATA_REPORT §五 第 6 条），
直到判据稳定且抽检完成。真实轨报告的定位是「如实标注边界」，不是「刷分」。

## 五、前端设计（漂亮 + 可交互）

现状：`scripts/showcase_app.py` 8 页签 Streamlit 门户（「数据净水厂」语言，CSS 定制、
`tests/test_showcase_app.py` 有渲染冒烟）。**工业页（第 5 页签）只读一份静态报告**——
用户点不动、看不到阈值怎么影响结果，这正是「可交互」的缺口。

### 5.1 数据预计算（把重活挪出交互回路）

新增 `scripts/build_real_interactive.py` → `data/reports/real_sensor_interactive.json`：

| 字段 | 内容 | 消费方 |
|---|---|---|
| `datasets[key].windows` | 每窗：`{channel, t, score_old, score_new, label}` | 阈值滑块 / 判据开关的即时重算 |
| `datasets[key].curves[op]` | 阈值网格 → `(recall, kill_rate, n_eval)` | 召回-误杀 曲线 |
| `datasets[key].kills[op]` | 误杀清单（通道 + 时间 + 读数极值 + 分数） | 抽检表 + 导出 |
| `meta` | 生成命令、判据版本、窗口宽、数据规模 | 口径自证 |

体积控制：分数与标签按窗存（数万条），**不存读数数组**；读数极值只在误杀清单里存。
理由：Streamlit 每次交互重跑脚本，**万行级 JSON 不缓存会放血**（`cached_verdicts` 同款教训）。

### 5.2 新页签「真实数据」（第 9 页签）

四项交互，全部即时响应（数据已预计算，不现场跑算子）：

1. **数据集切换**（SKAB / MetroPT-3 / C-MAPSS）+ 窗宽切换（SKAB 有 256/64 两档）
2. **判据版本开关**（旧判据 ↔ 新判据）→ 并排显示 `召回 / 误杀 / 存活率` 三格读数
   ——这是最有说服力的一幕：**同一份真实数据，改判据前后误杀 26.27% → X%**
3. **阈值滑块** → 实时重算该阈值下的 `(召回, 误杀)` 并标在召回-误杀曲线上
   （曲线由 `curves[op]` 画，滑块位置即当前点；用 `st.altair_chart` 或内置 chart）
4. **误杀清单表**（按通道/时间可筛）+ 一键下载 CSV

诚实声明（页内固定文案，不可省）：
- 「误杀率是**上界**：真实数据无标签 ≠ 干净」
- 「与合成轨数字**口径不同**，不可横向对比」
- 「真实轨数字**未进** `claims.json`」

### 5.3 视觉升级（「漂亮」的落点）

保留「数据净水厂」语言与洁净室蓝绿，做三处提升：

| 落点 | 做法 |
|---|---|
| 首屏读数仪 | 四领域卡改为**带迷你趋势条**的仪器面板；由 **红色=劣化 / 绿色=健康**？→ **否**：本项目是数据质量门禁，沿用现有语义色（`pass` 绿 / `fail` 红）**不套股票涨跌约定** |
| 曲线区 | 召回-误杀曲线用**面积 + 当前点标注**，不用默认样式 |
| 过渡动效 | 仅在读数变化时做 200ms 数字翻转；**不做**页面级动画（面试投屏要稳） |

⚠️ 顺手修正一处既有口径错误：`showcase_app.py` 的 `DOMAINS["image"]["blurb"]` 写
「**11 级滤芯**」，而实点 `OP_COUNT = 29`（跨四模态注册表）——两处口径打架，随本次一并改。

## 六、任务拆解与执行顺序

```
R2（最小、零风险，纯语义修正）
  → R6（抽检 + 扫描工具：V1/V2/V3 的度量器必须先有，否则后面改动无法验收）
  → R1（stuck：最容易出数，直接打 C-MAPSS 92.4% 误杀）
  → R3（fault_vs_maintenance：需要 ingest 配合产 gap 记录）
  → R5（合成污染器补形态：给 R1/R3 配合成回归靶子；③号负样本补误杀分母）
  → R4（drift 换尺度：影响合成门禁，放最后，且带回退方案）
  → F1/F2（预计算 + 页签 + 视觉升级；数据在 R1–R4 定型后一次生成）
```

**每项收工必附**：该项对应的真实轨前后对照数（V1/V2）+ 合成轨回归数（不倒退）。
**禁止**「判据改了但没数字说它更准」。
...
| # | 决策点 | 建议 |
|---|---|---|
| **1** | **修复范围** | **六项全做**（R1–R6）。理由：R6 是度量器，R2 是语义修正，两者是其余各项的前提；只做子集会导致「改了但无法验收」 |
| **2** | **前端形态** | **升级既有 Streamlit 门户**（加第 9 页签 + 视觉升级）。理由：门户已有 8 页签、CSS 语言统一、有 AppTest 冒烟；另起一个 HTML 单页会造成两套门面、两处口径 |
| **3** | **合成数字回退时的取舍** | 若 R4 使合成误杀 >1.23% 基线：**默认回退为 `pooled σ`，把 MAD 变成可选档**（保既有 config 与门禁数字不动） |

**三处已由用户确认（2026-09-20）**：① 修复范围 = **六项全做 R1–R6**；
② 前端形态 = **两者都要**（升级 Streamlit 门户**且**另出自包含交互式 HTML 单页）；
③ R4 回退策略 = **可选档**（默认仍 `pooled σ`）。

## 八、执行实点与设计修订（边做边记，2026-09-20）

### 8.1 已完成项与实点数字

执行顺序按 §六，实际推进 `R2 → R6 → R1 → R3`。**合成门禁全程逐位锁在基线**：
`123/0、128/8、184/20、63/0、57/0`，召回 100.0%（3 seeds 最差）、误杀 **1.23%**。

| 项 | 真实轨实点（前后对照） |
|---|---|
| **R2** `sensor_range` 表外通道记 `None` | 三个数据集的量程**从未评过**现在可计算：SKAB 未评 1048/1048、C-MAPSS 29757/29757、MetroPT-3 41475/41475（旧版一律报「0 误杀 = 通过」） |
| **R1** `sensor_stuck` 批量 + 无信息通道豁免 | C-MAPSS 误杀 **30.58% → 2.44%**（未评 8406 窗 = 6 个无信息通道被豁免的可计算证据）；MetroPT-3 首次真的抓到故障（命中真脏 300，旧版 0） |
| **R3** `fault_vs_maintenance` 三形态 + 设备级停机豁免 | MetroPT-3 stuck 误杀 **4.36% → 1.07%**（437/40659），且 300 次真脏命中**一条不丢**；`fault_vs_maintenance` 从「恒过」变成 707 窗记 `None`（诚实计量） |

### 8.2 ⚠️ R3 设计修订：不产 gap 记录，改为窗内跨度自检验（实点推翻了原设计）

原设计（§二 R3）写的是「`ingest_real_sensor.py` 按 `sampling_hz × 窗长` 推算应有槽位
网格、缺失槽生成 `record_type: "gap_window"` 样本」。**先量后改，探针把它否掉了**，
两条实点依据：

1. **payload 里的字段已经够算**：`window_start / window_end / sampling_hz / len(readings)`
   都在，`span_ratio = (end-start) / ((n-1)/hz)` 算子侧就能算 → 不需要新 record_type，
   也就不需要动包的 `KNOWN_RECORD_TYPES`（**少一处协议变更**）。
2. **「时间跨度异常」不是链路故障，是合法停机**：MetroPT-3 上 `span_ratio > 5` 的窗
   101/5925（1.70%），其中带故障标签 **2 个 = 2.0%**，与全窗口基准故障率 **2.0% 相同**
   → 与故障**统计独立**；按天聚合 91 天几乎每天 1 个、按小时聚合集中在 19 时(29)/0 时(20)
   → 是**每日停机**。把它判成「计划外静默 = 故障」会白杀 707 窗/通道。

所以 R3 的落点改成**算子侧三形态并集**（①全哨兵 ②设备级停机 ③跨度缺口），
且三者的裁决**互不相同**：① 计划内 1.0 / 计划外 0.0（合成红线，一字不动）；
② `1.0` + 证据 `machine_stop`（评过、是停机）；③ `None` + 证据 `span_gap`
（窗跨接两个运行时段，读数不构成连续观测 → 该问题**没有定义**）。

### 8.3 新增档：设备级停机（R3 顺带，误杀降幅最大）

判据：同一 `(device_id, window_start)` 下**全部通道读数恒定**（≥2 通道），且**不是**全哨兵形态。
真实数据里 `operating_mode == "idle"` 这个**标签**基本不存在，但「多通道同时冻住」是设备停机的
数据印记 —— **同步性**就是它与单通道卡死的唯一区别。

MetroPT-3 实点：**191 窗 × 7 通道 = 1337 条记录**，故障标签 **0/191**、检修计划内 **0/191**，
聚成 **7 段连续块**（5.1h ~ 61.4h，最长 61.4 小时）。三件事同时成立 → 是「设备没在跑」，
不是「传感器坏了」。旧判据把这 1337 条判成 `sensor_stuck`（传感器故障）是**误诊**。

两个算子对同一事实给不同答案是**对的**，不是不一致：
- `sensor_stuck` 问「这通道卡死了吗」→ **不适用**（不是单通道现象）→ `None`
- `fault_vs_maintenance` 问「这窗是计划外静默故障吗」→ **不是，是停机** → `1.0`

两者共用 `_device_stop_keys()`（单一真相源）。

### 8.4 新增能力：判据**适用性**检查（补齐 F1 空转的可计算分母）

`judge_reachability`（R6）回答「**门槛够不够**」，新加的 `judge_applicability` 回答
「**靶子有没有**」—— 同一句话的两个分母。MetroPT-3 实点：

| 算子 | 目标形态 | 命中 | 结论 |
|---|---|---|---|
| `fault_vs_maintenance` | 全哨兵静默窗 | **0** / 41475 | **无适用性（空转）** —— 真实数据里一个哨兵都没有 |
| `unit_consistency` | 同 `(device_type, channel)` 出现 >1 种单位 | **0** / 7 组 | **无适用性（空转）** —— 单位是采集侧统一填的，判据无区分度 |
| `sensor_stuck` | 严格平坦窗 | 1337 / 41475 | 适用（其中 191 窗是设备停机 → 记 `None`） |

这一节把 REAL_DATA_REPORT 里「五算子全静默」这类断言换成了**算出来的数**。

### 8.5 一次性探针（收工删除）

`scripts/probe_span_ratio.py` 是 R3 的决策探针，**收工删除**——避免造成第二个真相源
（`span_ratio` 的口径现在只应存在于 `industrial_quality._span_ratio`）。**已删除。**

### 8.6 前端 F1/F2/F3：实点与两处设计取舍

| 项 | 产物 | 实点 |
|---|---|---|
| F1 预计算 | `data/reports/real_sensor_interactive.json`（2.39 MB） | 三数据集 × 窗级时间线（**不存读数数组**，控体积）+ 曲线 + 误杀清单 + 适用性/可达性 |
| F2 门户 | `scripts/showcase_app.py` 第 9 页签「真实数据」 | 数据集切换 / 旧↔新并排读数 / 召回-误杀权衡面（altair，带当前工作点）/ 误杀清单可筛+导出 CSV / 窗级三分解 / 适用性+可达性表 |
| F3 单页 | `docs/real_data.html`（2.42 MB） | 零外部依赖（无 CDN/字体/图表库），模板抽成 `scripts/templates/real_data_page.html` |

**两处设计取舍（与 §5.2/§5.3 的偏离，均已在代码里写明理由）**：

1. **「阈值连续滑块」改成「预计算网格档位选择器」**。原设计要滑块实时重算 `(召回, 误杀)`。
   实际网格只有 `z ∈ {2,3,4,6,8}` × `scale ∈ {pooled, mad}` 十个点，
   网格之外的 z **没有数据**——给一个连续滑块就是在暗示「这些值也算过」。
   故改用 `select_slider` 只在十档上取值，并在页内写明「滑块在预计算网格上取值，不是现场重跑算子」。
2. **「迷你趋势条」改成「门禁量规条」**。报告里**没有时间序列**，画趋势就是编数据；
   改成「当前召回在 0–100% 上的量规」（填充 ∝ 召回，量纲正确），
   语义色沿用 `pass` 绿 / `fail` 红，**不套股票涨跌约定**。

**顺带修掉的设计表自身错误**：§5.2 写「SKAB 有 256/64 两档」并要求做窗宽切换——
256 档确实存在（`data/raw/real/skab/windows.jsonl`）但**不可用**，见 §8.7。

### 8.7 ⚠️ 待查：SKAB 256 窗档是「惰性」的（已调查，不进前端）

补 SKAB 256 档（`windows.jsonl`，1048 窗）跑了一次 R4 网格扫描
（`data/reports/real_skab_r4grid.json`，命令与 64 档同轴）：

| 档 | 窗数 | 干净/脏 | `sensor_drift` 丢弃 | 真脏命中 | 其余四算子 |
|---|---|---|---|---|---|
| SKAB 256 | 1048 | 536 / 512 | **0** | **0** | 全部 0 |
| SKAB 64 | 4512 | 2520 / 1992 | 1096 | 479 | 全部 0 |

即 **256 档上五个算子全部零动作**（既不是误杀也不是命中），而标注脏率 48.9% 并不低。
这不是本次改动引入的（`data/reports/real_skab.json` 早就是同一结果）。

**原因未定，故不进前端**——把一份「什么都不发生」的数据集放上页签，
面试官一问「为什么」就答不上来。列为待查项，三个候选方向：
①`operating_mode` 在 256 档缺失导致分组退化；②窗口更长 → 窗均值更稳 → 与基线的 z 更小；
③标签对齐口径在 256 档不同。
**结论口径**：在查清之前，SKAB 只以 64 档出现在前端，并在页面 caption 里如实标注窗宽。

### 8.8 两处「口径自相矛盾」的修复（都不在原设计里，是顺带实点炸出来的）

| # | 现象 | 实点 | 修法 |
|---|---|---|---|
| 1 | 门户 `DOMAINS["image"]["blurb"]` 写「**11 级滤芯**」，同页总览 `OP_COUNT = 29` | 注册表模态是 `image_caption / text_article / fhir_resource / industrial_sensor` 四值，**图文实点是 12**（8 纯图文 + 4 双模态）；**11 恰好是 `text_article` 的数量** —— 两个数字**串了档** | 改成 `_modality_counts()` **从注册表现算**，取不到就退回定性说法；加测试把「blurb 数字 == 注册表实点」绑死 |
| 2 | 我写的预计算 JSON 里 `state_code = {"0":"未评","1":"通过","2":"丢弃"}` | 实际 `d`/`d0` **只有 0/1**（通过/丢弃），「未评」在另一个字段 `u`；且**没有任何消费者读它** | 删除，换成如实描述各数组编码的 `columns` 字段 |

**注意**：若照抄「29」去改第 1 条**同样是错的**（29 是跨模态总数）——
这正是「先实点、别信描述」的用处：设计表说「11 与 29 打架」，实点结果是**两个都不对**。

### 8.9 收工账（2026-09-20 P2 修复 + 前端）

- 质量门：`ruff check` 双仓全绿；主仓 **414** + 包 **67**（0 失败/0 错误/0 跳过，`--junitxml` 实点）。
  387 → 414 = R 系列 **+15** + F2 **+12**。
- 硬约束核对：**未改 `Sample`/`Operator` 协议签名**；**既有 4 个 config 一字不动**
  （新增 `configs/funnel_industrial_real.yaml`）；测试基线**只增不减**。
- 产物：F1 JSON / F2 第 9 页签 / F3 `docs/real_data.html` / `real_skab_r4grid.json`（待查项证据）。
- 未提交（协作方在途，**不得 `git add -A`**）：`docs/PROOF_CHAIN.md`、`docs/claims.json`、
  `scripts/verify_claims.py`、`tests/test_verify_claims.py`、`Makefile`、
  `benchmarks/capability_matrix.json`、`runs/experiments.jsonl`、
  `scripts/eval_{fhir,industrial,judge,random_drop_baseline}.py`、`scripts/finetune_clip.py`、
  `src/mm_curation/tuning/extraction.py`。
- **待 glm 同步的口径**：真实轨数字（R1–R6 后的新误杀率）尚未进 `PROOF_CHAIN.md` / `claims.json`；
  本轮**没有**把真实轨数字写进任何「能力声明」文件——它现在的定位仍是**适用性证据**。

### 8.10 ⚠️ 真实轨召回低的根因实点 + 业界对标 + 修法候选（**待用户确认，未动码**）

**触发**：用户追问「命中率也太低了，到底是什么原因，对标一下业界的成熟方案」。
本节的数字全部现场重跑得到，可复现命令见文末。**本节只出结论与候选，不改任何代码。**

#### 一、四条根因（按影响排序，全部实点）

| # | 根因 | 实点证据 |
|---|---|---|
| **A** | **真值标签语义与算子职责不同源**（最主要） | 三个数据集的 `labels["dirty"]` 分别是 `degraded`(4200) / `fault_window_air_leak`(819) / `sensor_anomaly`(512)——**没有一个是「数据质量缺陷」**，全是过程/设备异常。C-MAPSS 上 `sensor_stuck` 对 4200 条脏只抓到 **124（3.0%）**，**2864（68.2%）"评了但过"**，**1212（28.9%）未评** |
| **B** | **判据的基线长度假设在真实数据上不成立** | C-MAPSS 的 `(device_id, channel, mode)` 组共 4200 组，**每组窗数中位数 6（min 4 / max 17）**，而 `sensor_drift._BASELINE = 5` → 中位情况下**只有 1 个窗可判决** |
| **C** | **判据族是单变量，信息维度不够** | AUC 对照（同数据同 σ̂，只换统计量）：电平偏移 **0.6272** / 趋势斜率 **0.6333** / 两者取大 **0.6254** —— 全部 ≈ 不可分。**加趋势项不解决问题** |
| **D** | **缺元数据 → 部分算子结构性空转** | `sensor_range` 在 C-MAPSS 上 **未评 29757/29757 = 100%**（真实通道名 `s1..s21` 不在内嵌量程表里）；`unit_consistency` / `fault_vs_maintenance` **100% 评了但过**（无靶子） |

**A 是结论性的**：退化的传感器读数本身完全合法（机器坏了、传感器没坏），
所以「漏掉的 78%」里绝大多数**读数上不存在任何数据质量缺陷**。
这不是召回问题，是**分母问题**。

#### 二、业界对标（成熟方案怎么做，我们差在哪）

| 业界成熟做法 | 代表实现 | 我们的差距 |
|---|---|---|
| **把「数据质量」与「过程/设备异常」建成两条独立流水线**，并明确区分 *data anomaly* vs *process anomaly* | 制造/工业界的共识表述：数据质量维度取 accuracy/precision/consistency/completeness/timeliness；而 dropout/异常检测是另一条线，且「**异常检测方法很少对细微数据错误有效**」——两者互补不可互替 | **我们混在一个召回分母里** → 分子分母不同源。这是 A 的行业依据 |
| **DQ 强依赖元数据**：量程、单位、时间戳有效性、schema、**资产层级**（Enterprise→Site→Line→Machine→Component→Measurement） | 工业数据平台的标准检查清单：类型/在物理可能范围内/必填字段/时间戳合理性/**工程单位**/**意外冻结值**/**变化率过大** | 我们**没有从真实数据做 profiling**（量程表靠手写内嵌） → `sensor_range` 100% 未评 |
| **带 verdict 的标记，而非删除** | 业界样例：`{"value":4500,"quality":"uncertain","validation_error":"outside_expected_range"}`，**保留原始事件**供下游判断 | ✅ **这一条我们做对了**：`verdict.jsonl` + `evidence.rule` + `None≠1.0` 与业界同向。**这是可以主动讲的优点** |
| **DQ 指标本身当运营 KPI 并监控**（`bad_quality_events_total` 等，按站点/设备/资产类型分组） | Deequ / 工业平台 | 我们**算了未评/误杀，但没把它变成门禁** → 「零动作」能静静通过 |
| **约束自动生成 + 指标时序异常检测** | Deequ：`ConstraintSuggestionRunner` 从 profiling 自动建议约束；`addAnomalyCheck` + `RelativeRateOfChangeStrategy` 对**指标时序**做异常检测 | 我们都缺：约束靠手写；算子分数**没有跨天/跨批次的时序视角** |
| **多变量统计过程控制**（PCA → T² + SPE/Q 残差 + 贡献图） | MSPC，1991 Kresta/MacGregor 起沿用至今；文献明确：「**即使单变量都在历史范围内，缓慢漂移的传感器也会让 SPE 升高**」；**贡献图可判故障归属**：单个变量主导 SPE → **传感器故障**；多变量同时贡献 → **过程故障** | 我们**只有单变量** → 根因 C。而贡献图那一条**正好是 A 的自动化解法** |

#### 三、修法候选（按「性价比 / 风险」排序，**待确认后才动码**）

| 序 | 修法 | 解决 | 风险 | 说明 |
|---|---|---|---|---|
| **1** | **拆分母**：真实轨只对「数据质量型」标签算召回；过程型标签另立一列「顺带发现率」，并显式标注**非设计目标** | A | **零**（只改报告口径，不改判据） | 最该先做。同时给三个数据集标注「标签属于 DQ 还是过程异常」——这是一处需要人工判定的**领域判断**，不能自动推 |
| 2 | **零动作门禁**：某算子在全数据集未评率 > 阈值，或**全数据集零动作**时，报告结论标 `INVALID` 而不是「保留 100%」 | D + 「零动作伪装成干净」 | 低 | SKAB 256 档会立刻被拦下（1048 窗五算子全 0） |
| **3** | **基线长度自适应**：`_BASELINE` 从常数改成 `min(5, max(2, n_windows // 3))` | B | 中（要重跑合成门禁逐位核对） | 现有 4 个 config 一字不动的前提是**默认值不变**；此改动会改默认行为 → 需先在设计塔里定「默认怎么取」 |
| 4 | **元数据/profiling 步**：从真实数据自动剖面出量程/单位/schema，替代手写内嵌表 | D | 中 | 业界叫 data profiling / constraint suggestion。量程必须**可追溯**（数据集文档/传感器手册），查不到的留空 = None |
| **5** | **加 MSPC 层**：PCA → T²/SPE + 贡献图，顺带用贡献图自动区分「传感器故障 vs 过程故障」 | C + A | **高**（新增判据族，要配对照实验与合成靶子） | 这是「让架构完整」的一层，价值最大但要按 W 系列那样**先设计塔、后动码**，且**必须有对照实验出数** |
| 6 | DQ 指标时序异常检测（Deequ `addAnomalyCheck` 的路子） | 补能力 | 中 | 需要多次运行的指标落库（我们已有 `runs/experiments.jsonl` 的口径基础） |

**建议顺序**：`1 → 2 → 3 → 4`（都是「把已有能力的口径修对 / 让空转可见」），
**5 单独立项**（属 V6「让架构完整」层，需对照实验）。
...
python -X utf8 scripts/eval_real_sensor.py --source skab --out skab_w64.json   # w64 档
python -X utf8 scripts/eval_real_sensor.py --source cmapss --sweep sensor_drift
```

**未落库的探针**（一次性，跑完即删；结论已写进 `ENGINEERING_NOTES.md` #71–#74）：
`probe_attrib.py`（读 `evidence.rule` 做互斥归因）、
`probe_slope_vs_level.py`（电平 vs 趋势的 AUC 对照）。
→ 若第 5 项（MSPC）立项，**AUC 对照必须固化成 `scripts/` 下的正式对照脚本**，不能停留在临时探针。

#### 五、口径提醒（别再写错）

- **真实轨召回不是能力指标**，在标签同源问题解决前它测的是「DQ 判据能不能顺带发现过程故障」。
- **「保留 100%」不等于「数据干净」**，可能是零动作（SKAB 256 档的 1048 窗就是例子）。
- C-MAPSS 的 `window_start` 是 ingest 造的**相对占位时间轴**（17 个取值，30h 步长），
  可排序但不是真实工况时间。

---

### 8.11 R7–R11：真实数据「清洗质量」的六项修法（实点，含一处设计被推翻）

> 承接 §8.10。用户 2026-09-22 指令原话：**「帮我继续完善一下重点解决没法清洗数据这件事，
> 不需要结果多么好看但是清洗质量不要这么差，可以参考已经有的项目」** → 批准按 §8.10
> 建议顺序动码。本节记实点与改口径的地方，**负面结果一并记**。

#### 一、六项修法的落地状态

| 项 | 内容 | 状态 |
|---|---|---|
| R7 | 基线长度自适应（`_BASELINE=5` 在每组 6 窗时只剩 1 窗可判决） | **未做**（理由见 §五） |
| R8 | 元数据/剖面：`sensor_range` 的**第二条边界来源**（数据包络 + 留出集自检） | ✅ 本节 §三 |
| R9 | MSPC 多变量层 `sensor_multivariate`（补**维度**，不是更好的阈值） | ✅ 已进真实档 `funnel_industrial_real.yaml` |
| R10 | 覆盖率闸门：把「未评」与「零召回」分开，禁止把「保留 100%」读成「数据干净」 | ✅ 本节 §四 |
| R11 | 半合成注入评测（给真实数据造**有效真值**） | ✅ 本节 §二 |
| — | 顺带修掉一处**真实的**分母口径错误（背景率分子含被替换的原窗） | ✅ `ENGINEERING_NOTES.md` #76 |

#### 二、R11：`scripts/eval_real_injection.py` —— 真实背景上的半合成注入评测

**为什么必须新造真值**：三个真实数据集的标签（C-MAPSS `degraded` / MetroPT-3
`fault_window_air_leak` / SKAB `sensor_anomaly`）**全是过程/设备异常**，没有一个提供
数据质量真值（§8.10 根因 A）。拿它们当召回分母 = 用 DQ 检测器去考故障诊断。

**做法**（业界标准，CWRU/SKAB/MIMII 均如此）：基线跑 → 注入已知缺陷再跑 → **差分口径**：

| 量 | 定义 | 能不能对外讲 |
|---|---|---|
| `recall_injected` | 注入窗被丢 / 注入窗总数 | ✅ **有效真值**（分母真值已知） |
| `collateral_false_drop` | 注入跑里新丢的**非注入**窗 | ✅ 注入造成的附带损伤，应 ≈ 0 |
| `background_flag_rate` | 基线跑丢的非注入窗占比 | ⚠️ 是「这份数据**自带**的异常率上界」，**不是误杀率** |

三个数**不许混**，报告里强制分列。

**只注入数据质量型五形态**（`flatline` / `cal_offset` / `out_of_envelope` /
`unit_mismatch` / `silence`），刻意不注入过程异常 —— 那不在本项目职责内，注进去
才是自己骗自己。**不复用** `curation_eval.sensor_contamination` 的现成污染器：
它们隐式依赖合成语料形状，在真实窗上供体池直接为空（`_beyond_baseline` 要 ≥5 个更早窗、
`out_of_range` 要内嵌量程表、`unit_swap` 只认 `MPa`）→ 与根因 A/D 是同一件事。

#### 三、R8：数据包络 + **留出集自检**（一处设计被 MetroPT-3 推翻）

`sensor_range` 在三个真实数据集上**一次都没评过**（通道全在手册量程表外，记 `None`）
→ 注入 1888 条已知越界缺陷召回 **0.0%**。所以问题不在阈值，在**缺输入**。

**边界怎么定**（`scripts/build_envelopes.py`，四条纪律）：
1. **只看参考段**（每组最早 `ref_frac=0.3` 比例的窗，工控 golden batch 口径）；
2. **读数级**（判据逐读数比边界；若用「窗均值 ± 窗均值散布」，宽度小一个 `√n` 量级 → 疯狂误杀）；
3. **跨设备/工况取并集**（最宽）——个体差异与工况差异属合法范围，只抓「比所有设备都更极端」的值；
4. **带 `n_ref`**，低于 `min_ref`（15）的边界**不许用**（判据仍记未评）。

**被推翻的设计（最有价值的一条）**：上面四条在 C-MAPSS / SKAB 上很好
（包络自身背景率 1.16% / 0.66%），但同样的配方在 **MetroPT-3** 上让
**54.40% 的真实窗越界**（漏斗级 56.29%）—— 一半以上的真实数据被判成脏。
根因：固定参考段隐含**「数据稳态」这个未声明假设**；MetroPT-3 是真实压缩机，
负载/启停在持续变化，前两个数据集（仿真退化 / 小试验台）**掩盖**了这个问题。

**修法**：给**生成端**加**留出集自检** —— 边界仍只由参考段**拟合**，
但用参考段**之外**的读数做接受/拒绝检验（留出集验证，不是「用整份数据调边界」，
不构成同义反复）：留出集越界率 > 5% 的组，**整条边界不写**，宁可让判据记未评。

> MetroPT-3 的 `Motor_current` 留出集越界率 48.9% → 该组被拒。
> 效果：包络背景异常率 **54.40% → 0.43%**，漏斗注入召回 **81.2% → 89.9%**
> （不再被误判淹没）。C-MAPSS / SKAB 的召回不受损。

#### 四、十项实点（三数据集完整档，`seed=42`、注入率 10%）

**漏斗级（串联）**：

| 数据集 | 读数窗 / 背景 / 注入 | 注入召回（无包络 → 自检包络 m6） | 附带误丢 | 背景自带异常率 |
|---|---|---|---|---|
...
|---|---|---|---|
| `flatline`（卡死） | 76.5% | 97.6% | 100.0% |
| `cal_offset`（校准偏移） | 87.6% | 97.1% | 81.4% |
| `out_of_envelope`（越出包络） | 62.2%（原 26.2%） | 54.9%（原 11.4%） | 84.8%（原 59.3%） |
| `silence`（链路静默） | 100.0% | 100.0% | 100.0% |
| `unit_mismatch`（单位混用） | 100.0% | 100.0% | 100.0% |

**每个形态都有算子实质覆盖它** —— 这是「清洗能力真的存在」的可核对证据，
与 §8.10 那个「21.9%」（分母错配）形成对照。

**算子级 `sensor_range`**（R8 的净效果）：

| 数据集 | 无包络 | 自检包络 m6 | 覆盖率 |
|---|---|---|---|
| C-MAPSS | 0.0%（`ALL_UNSCORED`） | **35.1%**，附带误丢 0 | 67% |
| MetroPT-3 | 0.0%（`ALL_UNSCORED`） | **25.2%**，附带误丢 0 | 43%（`LOW_COVERAGE`） |
| SKAB w64 | 0.0%（`ALL_UNSCORED`） | **43.5%**，附带误丢 0 | 100% |

**margin 扫档**（C-MAPSS 25 设备冒烟档，`4/6/8/12`）：漏斗召回在 margin ≥4 后
**饱和**（83.2–83.7%），但包络背景率随 margin 单调下降（3.29% / 1.10% / 0.77% / 0.35%），
SKAB 上 `margin=6` 与 `8` 结果相同且背景率仅 0.12%。
→ 定 **`margin=6`**：注入的越界幅度（`p99 + 10×IQR`）远超 6×MAD，加严只损失覆盖率不换召回。

**合成门禁回归**（`eval_industrial.py --seeds 42,7,2026`）：`123/0、128/8、184/20、63/0、57/0`，
召回 **100.0%**、误杀 **1.23%** → **逐位锁基线**，证明改造对既有 4 个 config 零影响
（`envelope_path` 不传时走原逻辑）。

#### 五、R7 未做的理由（如实标注）

`_BASELINE=5` 的问题是「每组只有 6 个窗时只剩 1 个可判决窗」。
但本轮实测显示：**真实数据的主要瓶颈不是基线长度，而是「稳态假设」本身**
（§三）。把基线从 5 改成自适应，是在**同一个错误假设**上做微调 ——
正确的方向是**滚动基线**（用最近 N 个窗，而非全寿命最早的 N 个），
那是 MSPC/SPC 里另一套设计（EWMA / 移动窗），需要单独的对照实验与设计表。
**故本轮不做，不假装做完了。**
...
python -X utf8 scripts/build_envelopes.py --source skab --windows windows_w64.jsonl --margin 6 --out /tmp/env_skab.json
python -X utf8 scripts/build_envelopes.py --source metropt3 --margin 6 --out /tmp/env_mp3.json
...
- 上面 §8.10 三条继续有效。
- **新增**：报「召回」时必须同时报**已评覆盖率**（R10）。一个覆盖率 0% 的算子，
  它的 `0.0%` 不是零召回，是**没有分母**。
- **新增**：包络类是**代理边界**，不是物理量程 —— 它只能发现「偏离自身历史形态」的值，
  发现不了「一直在量程外但稳定」的值（那种要靠手册量程）。
  且**同一个 margin 不能跨数据集搬运**（MetroPT-3 就是反例）。
