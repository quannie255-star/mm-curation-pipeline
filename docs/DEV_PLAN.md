...
**测试基线**：主仓库 **191** + 包 **40**（2026-09-15 OPS w1 会话实点 `pytest --collect-only`，本机含 ray 口径；未装 ray 的环境/CI 包侧为 35）；ruff check 全绿（唯一报错在协作方在途 findata_health_stage.py，在案未触碰）。任何提交不得低于该基线。
