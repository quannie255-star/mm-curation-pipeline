# 项目路线图：多模态数据质量平台（清洗管道 → 向量检索 → 个人微调）
...
开发日志全程见 docs/DEV_PLAN.md；面试弹药库见 docs/ENGINEERING_NOTES.md（59 条）。
...
两次中间阴性（v1 -0.180 / v2 -0.007）的根因链与修复见笔记 #58-59：
训练/推理协议错位 + completion 被截断在窗外（loss 曲线健康的静默任务替换）。
测试基线：主仓 157 + 包 40 全绿。
| 2026-09-03 | **V3 ζ 首战达标**：PRD 落盘（个人微调平台，锚点「专属数据判官」）+ 数据获取器（中新网 705 篇，robots/限速/幂等）+ benchmark 构建器（judge_news_v1 冻结 300 条，三重独立性保障）+ LoRA 微调器（peft+0.5B 本机 8GB）+ 评测 runner；**钱表：通用 κ=-0.024 → 微调 κ=+0.560（R=0.960，解析率 100%）**；两次中间阴性的根因链（协议错位/completion 窗外截断）进笔记 #58-59 | 首战闭环完成；扩语料（sitemap 源）、7B 租卡、第二数据域为后续 | 见 git log |

## V3 η：偏好闭环 + 迁移验证（2026-09-04，η-a/η-c 达标）

> 依据 PRD §九差距立项。η-a 回答「偏好能否进训练信号」；η-c 补验收 4 域外泛化欠账。

| 任务 | 内容 | 状态 |
|---|---|---|
| η-c | 域外泛化报告：judge_wiki_ood（维基 198 条，seed 9500 隔离 + 泄漏检查） | ✅ **κ 0.560→0.178**（P 0.706→0.546 / R 0.960→0.969）：召回跨域保持、精确率域锁定——「换域需重训，骨架可复用」 |
| η-a | 偏好闭环试点：详略偏好（PA 精炼派/PB 求全派）persona-oracle 协议 + 程序化 S/F 变体对 + 最小对 DPO（trl 0.17 + peft LoRA，0.5B 本机）+ 冻结偏好 benchmark（150 题，结构性排除 judge 数据源） | ✅ **PA 命中率 0.933 / PB 0.867 / 分歧率 0.783**（验收线 0.75/0.75/40%），通用基线 0.48/0.40 |
| η-b | 第二任务示范：抽取忠实性判官（客观 oracle：事实-原文对齐；三损伤分层） | ⚠️ 首轮未达标：微调 0.53/0.50/0.41 ≈ 通用基线——阅读对齐超出 0.5B 能力下限，转化为「判官能力矩阵」产品概念（笔记 #62）；重训选项 1.5B+ |

**η-a 钱表（pref_news_v1 冻结 benchmark，60 文档对 × 2 persona + 30 对照题）**：

| 判官 | 本 persona 命中率 | 对立 persona 命中率 | 对照题命中 |
|---|---|---|---|
| PA 精炼派判官 | **0.933** | 0.117（反向，符合设计） | 0.867 |
| PB 求全派判官 | **0.867** | 0.167（反向，符合设计） | 0.800 |
| 通用 0.5B（不微调） | 0.483 / 0.400（≈随机） | — | 0.27-0.53 |

**分歧率 0.783**：同一批文档，两个偏好训出的判官 78% 给出相反判定——
「偏好进了训练信号」的直接证据。两次中间失败的根因链（DPO 梯度被模板
token 吸收→最小对改造；解析截断伪装成学崩）见笔记 #60-61。
诚实边界：v1 偏好标注是 persona-oracle（规则模拟），非真人标注。
测试基线：主仓 162 + 包 40 全绿。

## V3 θ：偏好判官工坊——真人偏好闭环 + 大众可用向导（2026-09-06）

> 依据 PRD §九差距 1/3 立项：把 η-a 的 persona-oracle 换成真人 A/B 点击，
> 并给「数据→benchmark→训练→评测→试用」全流程一个非开发者可用的外壳。
> 设计表 docs/design_tables.md θ 节（用户确认，commit 1068e34）。

| 任务 | 内容 | 状态 |
|---|---|---|
| θ1 | 数据逻辑层：标注 v2 schema（候选全文落盘——旧工具只存字数、真人闭环事实上断裂）+ oracle 模拟用户生成器 + build（最小对/按对 holdout/REJECT 剔除/对照题/冻结考卷通道） | ✅ |
| θ2 | CLI：build_user_pref_data / build_oracle_labels + 评测报告按 benchmark 名落盘 + `--generic` 语义修正（旧版带 flag 会静默跳过 adapters） | ✅ |
| θ3 | `scripts/judge_studio.py` 五步向导（导入→标注→训练→评测→试用），冒烟 200 | ✅ |
| θ3.5 | 模拟用户 e2e：首轮 250 条（206 三元组）×2ep → **命中率 0.532 ≈ 通用基线 0.532，未达标**（欠训练：final loss 0.654、rewards/acc 0.557）；**加量到 550 条（536 三元组）×2ep → 命中率 0.839 ✅**（loss 0.468、rewards/acc 0.96）——考卷 77 题两轮原封未动（冻结考卷通道） | ✅ 达标 |

**θ 钱表（pref_user_v1 冻结 benchmark，62 main + 15 对照，两轮同卷）**：

| 判官 | main 命中率 | 对照题（损伤否决） |
|---|---|---|
| 通用 0.5B（不微调） | 0.532（≈随机） | 0.40 |
| 你的判官（488 对训练） | **0.839（+30.6pp）** | **0.80** |

**学习曲线（诚实数字）**：188 main 对 ×2ep → 0.532 未学会（≈通用）；
488 main 对 ×2ep → 0.839 达标——「最少标注量 ≈ 500 对」是当前证据下的
产品参数（向导建议量已按此更新）。真人账待用户实际点击后另报。

**诚实边界**：本表命中率均为模拟用户账（labeler=oracle，seed 53/57 可复现，
benchmark 产物不入库）。工程发现：泄漏检查在中文上的两层失真（模板前缀
假阳性 + 字节 4-gram 塌缩）见笔记 #63；「最少标注量」学习曲线见笔记 #64。

| 2026-09-06 | **真实脏数据试跑 + chinese_ratio 语义修复**：2066 篇真实爬取新闻过文本漏斗，暴露爬虫空白膨胀误杀（773/778，笔记 #65）→ 修 chinese_ratio 为去空白后计占比 → 重跑保留率 61.5%→**98.3%**，拦截精确命中审计预测（5 篇真非中文含 1 篇乱码），MinHash 转载对发现 17→29；框架启示：算子是纯打分器，归一化应在评分语义内解决而非加改写算子 | 真实数据双轨方法论闭环：合成给 P/R，真实给曝光 | 见 git log |

## V3 ι/κ/λ：对标调研后的三实验（2026-09-15）

> 对标 Data-Juicer / NeMo Curator / datatrove / dolma 等同类项目后立项（结论：
> curation-eval「回答算子好不好」生态位成立，Data-Juicer Juicer 模型正逼近域判官
> 方向）。三块均先落设计表（design_tables.md ι/κ/λ 节）再动码。
...
## V6 α：前置改写通道 + 归一化层 + 判决书（2026-09-20）

> 用户批准 V6 设计表（`docs/design_tables.md` V6 节）后的第一个执行期。**明确不追差异化**
> ——先把全链路缺层补齐，搭出来再看引入什么前沿技术。W1 = 「让证据可信」：改写通道 +
> 归一化层 + 判决书 + 2 个 UI 页签。

| 件 | 内容 | 结果 |
|---|---|---|
| 协议层（包 v0.5.0） | 新增与 `Operator` **并列**（非继承）的 `Transformer` 协议 + `TransformResult` 三工厂（unchanged/replaced/unusable）+ `run_pre_stages`；`run_funnel` 增 `pre_stages` 可选参数默认空 | **不改 Operator 签名、既有 config 一字不动**——不传 `pre_stages` 时行为逐位不变（V4「既有指标逐项相等」红线） |
| 归一化层（主仓） | 七条纯函数规则（NFC / U+2028 / `\r` / 零宽 / Cc-Cf / 空白折叠 / 首尾 trim）+ 语料级聚合 + `TextNormalizeTransformer` | 规则**正交**（逐规则归因纯）：`whitespace_collapse` 976 / `newline` 891；只声明自然语言模态，结构化载荷（canonical JSON）刻意排除 |
| 判决书（主仓） | `verdict.jsonl` schema v1：`rule` 机器可读判据代号 + `evidence` 判据真正用到的量 + `input_fingerprint` + `transform` + `review` 人审回填位 + PROV-O 三元；`wrap_for_verdict` 包装算子（`isinstance` 语义不变，**不动执行器**） | **旁路产物**：不传 `verdict_ledger` 时零落盘零痕迹（三条集成红线单测锁死） |
| A/B 对照实验 | `normalize_ablation.py`：A 原样 / B 归一化，两口径**各自独立装载**语料（算子会写 meta，复用会串味）；含模型档另存报告名避免互相覆盖 | 全量 2066 篇双跑**逐字节相同**；不变量断言「台账行数 = 各滤级进入数之和」 |
| UI（扩到 ⑧ 页签） | ⑦清洗过程（逐级水位对照 + 判决台账可筛滤芯/判决 + 「这跟业界差在哪」）/ ⑧阈值沙盘（分数分布 + **丢弃预算反推门限**，口径与误杀率显式分开；批量算子显示「不适用」） | 台账读取加缓存包装（Streamlit 每次交互都重跑脚本）；**修掉批量算子分支的 `NameError`**（指标卡曾渲染在 `if rec:` 之外） |

**读数（2026-09-20，全量 2066 篇）**：7 个规则档算子中**只有 `text_minhash` 判决变化**
（29 → 30，Δ=+1），存活 2032 → 2031；`chinese_ratio` Δ=0（#65 的单点补丁已覆盖它）。
归一化改写 **976/2066 篇（47.2%）**、净删 **55.8%** 字符。**空白占比 p50 1.98% → 1.94%
（几乎不动）而 p90 79.89% → 8.09%（差一个数量级）**，空白占比 >50% 的篇目 **849 → 0**
——语料是双峰的，归一化作用在长尾而不在中位数。

**如实标注的边界**：归一化层的确定增量**不在拦截数/召回上**（判据看不见它），而在
**下游载体**（进下游的字符总量、去重签名纯度、困惑度输入）与架构正确性。**不抢数字。**

**三条踩坑**（笔记 #69/#70/#71）：①框架缺的是**通道**不是算子——改写必须排在所有打分
之前，塞成算子会让前几级在脏文本上打分；②规则职责重叠会让逐规则归因失真，单测抓不到
（靠「每个统计量只有一个来源」的纪律）；③**追加式台账 × 整文件聚合 = 报告无声漂移**
——不改一行代码、重跑一次报告就涨一轮，已加不变量断言 + 双跑逐字节相同作为验收。

测试基线 **387 + 67**；ruff 双仓全绿。

## V6 W2 收官：抽取层 + RawDoc + 源接入协议化（2026-09-20）

> 这一层补的是笔记 #65 暴露的**真缺口**：`web_sources.py` 抓完 HTML 立刻抽成 text，
> **原始 HTML 从不落盘**。后果三条：换抽取器无法重放 / 抽取层没有可比基线 /
> 那次「空白膨胀致 778 篇误杀」的现场事后只能靠复述。

- **`RawDocStore`**（内容寻址存档）：`data/raw/html/<sha[:2]>/<sha>.html.gz` + `.meta.json`
  边车；幂等（同内容只落一份、只累积 urls）、原子写、`get()` 重算 sha 校验、
  不符抛 `RawDocCorrupted`（**不静默降级**）。60 篇实测：4,754,605 字节原文 →
  1,113,799 字节落盘（压缩比 **0.234**）。
- **三级抽取链**：`news_cn`（tier0 站点专用，stdlib）/ `trafilatura`（tier1，
  `requires` 探测 + 懒加载 + 未装记 `unavailable` 跳过）/ `heuristic`（tier2 stdlib 兜底，
  **永远可用、CI 零新依赖**）。`ExtractAttempt` **失败也记且记原因**
  （`no_container` / `no_paragraphs` / `too_short` / `unavailable`）。
- **W2-4 收编用「搬实现不复制」**：`NewsCnExtractor` 是唯一真相源，
  `web_sources.extract_article` 退化成薄适配 → **逐字等价是结构保证，不是测试负担**。
- **源接入协议化**：`SourceConnector`（`listing_urls` + `parse_listing`）三实现
  ——静态列表页 / RSS·Atom / Sitemap（`max_sitemaps` 上限，**不设上限等于对源站做全量拉取**）；
  `spec()`↔`from_spec()` 往返无损；`ingest()` 与旧 `crawl` 在**同一假 fetch 下产出
  逐字节相同的 JSONL**（离线断言）。

**对照实验（60 篇真实页面，`data/reports/extract_ablation.{json,md}`）**：

| 档位 | tier | 成功 | 关键交叉口径 |
|---|---|---|---|
| `news_cn` | 0 | **49/60**（11 篇抽不出） | 基准 |
| `trafilatura` | 1 | 未安装（**从分母剔除，不记 0 分**） | — |
| `heuristic` | 2 | **60/60** | 救回率 **100%（11/11）**、丢失率 **0%**；**字数比 1.92×、多抽段占比 88.9%** |

**结论不是「谁更好」，是三件不同的事**：兜底器保证**从不失败**（tier2 的价值即在此），
但**绝不能当默认**——它把侧栏推荐位 40+ 条标题抽进正文，会污染下游所有算子。

**顺带追出站点抽取器的两个真缺陷**（详见 `docs/ENGINEERING_NOTES.md` #66）：`<a>` 剥离
正则会删掉段落**内部**的行内链接文本（导语里「中新社」署名整批消失），跨段 `<a>` 会把
多段拼成一段。**而这条路径正是产出既有 2066 篇语料的那条路径** → 待修，但修它会改变
既有语料，必须配独立 A/B（已记待办）。

**新基线：主仓 387 + 包 67**；ruff 双仓全绿。
**未做**：人工抽检 30 篇只产出对照表（`extract_review_sample.jsonl`，`human_verdict` 待填）。

---

## V6 P2 收官：真实数据判据修复 R1–R6 + 前端 F1/F2/F3（2026-09-20）

> 用户指令：「**先解决处理清洗真实工业数据效果不好这一问题**，其他的可以先放一放，
> 然后给我一个**漂亮的可交互前端**」。三处待裁决经用户确认：修复范围 = **六项全做 R1–R6**；
> 前端 = **Streamlit 第 9 页签 + 自包含 HTML 单页两者都要**；R4 回退策略 = **可选档**。
> 设计表 §八 记录了执行中的三处设计修订与两处顺带修掉的口径矛盾。

**问题**（`docs/GAP_AUDIT.md` P2 档）：合成轨召回 100% / 误杀 1.23%，迁到真实数据后
**召回 24%~72%、误杀 16%~28%**。根因不是「阈值不对」——五档阈值扫描已证明调不动——
而是**五个工业算子的判据全都是对着合成注入形态写的**：那些形状在真实数据里要么
**不存在**（→ 空转，恒过），要么**同一个形状另有含义**（→ 误判）。

**六项修复与实点**（合成门禁全程逐位锁基线 `123/0、128/8、184/20、63/0、57/0`）：

| 项 | 改了什么 | 真实轨实点 |
|---|---|---|
| R2 | `sensor_range` 表外通道从 `return 1.0`（**伪装成通过**）改记 `None` | SKAB 1048/1048、C-MAPSS 29757/29757、MetroPT-3 41475/41475 首次如实显形 |
| R6 | 抽检导出 + 阈值扫描固化进 `eval_real_sensor.py`；新增**判据适用性**检查 | MetroPT-3：`fault_vs_maintenance` **0/41475 无适用性**、`unit_consistency` **0/7 组无适用性** |
| R1 | `sensor_stuck` 三级 → **四级判据**（⓪设备级停机 ①无信息通道豁免 ②严格平坦 ③持续尺度塌陷） | C-MAPSS 误杀 **30.58% → 2.44%**；MetroPT-3 **4.36% → 1.07%**，300 次真脏命中一条不丢 |
| R3 | `fault_vs_maintenance` 由「恒过」改**三形态并集**（哨兵 / 设备停机 / 跨度缺口） | MetroPT-3 191 窗 × 7 通道 = 1337 条设备停机被如实识别（旧判据全判成传感器故障） |
| R5 | 合成污染器补三类真实形态（噪声平坦线 / 采样停顿 / 信息黑障）+ 低脏率档 | 合成门禁逐位恢复基线；低脏率 2% 档 PASSED |
| R4 | `sensor_drift` 增 MAD 尺度（`max(1.4826×MAD, pooled)` 作地板 → 只会更宽松） | 同召回档 MetroPT-3 **28.90% → 7.24%**（降 75%）；全档 **28.90% → 3.98%**；C-MAPSS 18.96%→1.22%；SKAB w64 24.48%→1.03% |

**两条最有面试价值的现象**（详见 `ENGINEERING_NOTES.md` #67–#70）：

1. **同一个事实，两个算子给相反答案是「对的」**：MetroPT-3 上 1337 条全通道同步冻结，
   官方故障标签 0/191、检修记录 0/191、聚成 7 段连续块（最长 61.4 小时）→ 是**设备停机**
   不是传感器故障。`sensor_stuck` 答「不适用」（`None`），`fault_vs_maintenance` 答
   「不是故障，是停机」（`1.0` + 证据 `machine_stop`），两者共用 `_device_stop_keys()`。
2. **「注入即复制」让「连续 N 窗」类判据在合成轨结构性不可达**：污染器复制供体窗、
   保留原窗、两副本同 `window_start`，而算子按 `(window_start, id)` 全序排序 →
   游程每两窗重置，实测 12 通道最长游程仅 3~5（门槛 10）。**不是判据错，是合成轨表达不了它。**

**前端**：

- **F1** 预计算 `data/reports/real_sensor_interactive.json`（2.39 MB）——前端只做呈现，
  口径只有一处（前端自己算统计，迟早和报告对不上）。
- **F2** 门户第 9 页签「真实数据」：数据集切换 / 旧↔新判据并排读数 / 召回-误杀权衡面
  （面积 + 当前工作点标注）/ 误杀清单可筛 + 导出 CSV / 窗级三分解（**「通过但有算子未评」单列**）/
  适用性 + 可达性表。视觉升级：首屏读数卡加**门禁量规条**（填充 ∝ 召回）。
- **F3** `docs/real_data.html`（2.42 MB，**零外部依赖**：无 CDN / 字体 / 图表库，双击即开），
  含窗级时间线（每通道一 canvas）、手写 SVG 曲线、浅/深主题；模板抽成
  `scripts/templates/real_data_page.html`（同时根除 33 处 ruff E501，产物逐字节等价已验）。

**顺带修掉两处「口径自相矛盾」**：门户「11 级滤芯」实点是 **12**（**11 恰好是 `text_article`
的数量——串档**）→ 改为从注册表现算并加测试绑死；预计算 JSON 里无消费者且与数据编码
打架的 `state_code` 删除。**注意照抄「29」去改同样是错的**（29 是跨模态总数）。

**如实标注的边界**：SKAB **256 窗档** 1048 窗上五个算子**全部零动作**（64 档命中 479）——
原因未定，**故不进前端**（已列待查，详见设计表 §8.7）；真实轨数字**未进** `claims.json`
（定位仍是适用性证据，不是能力声明）。

**新基线：主仓 414 + 包 67**（387 → 414 = R 系列 +15 + F2 +12）；ruff 双仓全绿。
**未做**：`data/raw/real/`（383MB）仍未入 `.gitignore`（GAP_AUDIT P0-1 仍在，本轮未动）。

---

## JD 对齐：能力**外延**的三处缺口（2026-09-20）

> 背景：用户指示「对齐相关岗位的 JD 要求，继续完善项目」。重抓 10 个当前在招岗位
> （字节 Seed/阿里云 MaaS/阿里 Astar/阶跃星辰/B站/月之暗面/电子五所等，含 5 社招 + 5 校招），
> 聚类出 18 条要求，逐条按**仓库实点**映射。详见 `docs/JD_RESEARCH.md` v2。

**结果：✅ 有实证 8 条 / 🟡 部分具备 7 条 / 🔴 缺 3 条。**

值得单独记一笔的是**缺的这三条是什么**：

| 🔴 缺口 | 出现频次 | 性质 |
|---|---|---|
| 数据合成与增强（Data Synthesis / Augmentation） | 5/10 | 能力外延（现有只有**反向**的污染器） |
| 视频 / 音频模态（FFmpeg 抽帧 / ASR / Librosa） | 4/10 | 能力外延（现有四模态，无视频音频） |
| SQL / 数据建模 / 数仓 | 3/10 | 能力外延（有分析无资产化出口） |

**没有一条是清洗能力本身**——而「清洗/质检/去重体系」是 10/10 岗位都提的要求，
项目在这一栏是 ✅ 最密集的地方。这个分布本身就是结论：
**清洗这条线已经够深（29 算子 / 四模态 / 每算子可算 P:R），下一步该补的是出口。**

此外还有 7 条 🟡 部分具备，其中 6 条**已被 V6 计划覆盖**（采集解析→W2、血缘→W4、
配比→W4、断点续跑→W4、可视化→W5、Bad Case 归因→W3），只剩「数据回流飞轮」未覆盖。

**由此新增 5 个任务（J1–J5），已落 `docs/design_tables.md`「V6 补表 v2」，等确认后动码**：

- **J1 合成/增强通道（1→N 扩样）** —— 唯一需要协议变更的一项。W1 刚建的 `Transformer`
  只能 **1→1**（unchanged / replaced / unusable），而合成与增强是 **1→N**：
  **通道形状不对**。这与 W1「缺的不是算子，是一条通道」是同一类判断。
  附带一条红线：**合成样本必须走同一条漏斗**，否则「我合成了一批数据」不可证伪。
- J2 视频/音频模态 + 抽帧（复用图像算子链，扩模态的低成本路径）
- J3 数据目录 + SQL 查询层（DuckDB；验收要求「用 SQL 复现一个现有报告数字且逐位相等」）
- J4 语料分布体检报告（多样性/冗余/缺失/长尾，把散落片段收成一份）
- J5 公开评测生态互通（只做导出/导入互操作，不做全套集成）

**明确不追**（写下来比装看不见强）：EB/PB 实跑、Agent 行为数据与 RLHF 全链路、
论文级产出、PE/Agent 应用开发。

**执行顺序**：~~W2（进行中）~~ → **J1** → J3/J4 → W3 → W4 → W5 → J2 → J5。
（W2 已于 2026-09-20 收官，见上一节；下一步是 J1 合成/增强通道，**等用户确认后动码**。）
