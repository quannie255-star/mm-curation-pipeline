...
## V3 η：从「微调底盘」到「真平台」（2026-09-04 立项，进行中）

> 依据 PRD §九三条愿景差距立项（偏好未进闭环 / 任务域迁移零验证 / 产品化入口未做）。
> 设计门：docs/design_tables.md η 设计表（0e9795d），经确认后才动代码。

| 任务 | 内容 | 状态 |
|---|---|---|
| η-a | 偏好闭环试点：偏好操作化为详略维度（PA 精炼派 vs PB 求全派），persona-oracle 协议（v1 非真人标注，如实声明）；S/F 变体对 + 顺序随机化 + 内容对照题防长度 shortcut；DPO 主案（trl 未装=首要风险）+ SFT 退化案；工程红线沿用 #58/#59 | 🔄 进行中 |
| η-c | 域外泛化补账：wiki 域外 OOD benchmark（judge_wiki_ood），补 PRD 验收 4 欠账 | 🔄 进行中 |
| η-b | 第二任务示范（文风模仿判官 / 抽取质量判官），视 η-a 结果立项 | ⏸ 未立项 |

**验收线（η-a）**：双判官各自命中率 ≥0.75 + 分歧率 ≥40%（偏好的本质是「人和人
不一样」，双判官全对反而说明学成了客观题）+ 通用基线对照。
